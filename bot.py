...
@bot.tree.command(name="서버추가", description="현재 서버 채널을 봇 세팅 서버로 저장합니다. 직접 만든 채널도 자동으로 연결합니다.")
...
@bot.tree.command(name="임베드버튼붙이기", description="현재 채널의 봇 임베드에 원하는 버튼을 직접 붙입니다.")
...
@bot.tree.command(name="임베드버튼제거", description="현재 채널의 봇 임베드에서 버튼을 제거합니다.")
...
@bot.tree.command(name="임베드버튼자동설정", description="임베드 설정/전송 시 버튼 자동 연결을 켜거나 끕니다.")
...
async def auto_embed_button_setting_command(interaction: discord.Interaction, button_type: str, enabled: str):
    ...
    if not ok:
        return await interaction.response.send_message("❌ 이 버튼 종류는 설정할 수 없습니다.", ephemeral=True)
...
@bot.tree.command(name="임베드버튼목록", description="현재 서버의 임베드 버튼 자동 연결 설정을 확인합니다.")
async def list_embed_button_settings_command(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("🧩 **임베드 버튼 자동 연결 설정**\n" + "\n".join(lines), ephemeral=True)
...
class UnifiedPanelCustomEmbedButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not self.embed_type:
            return await interaction.response.send_message("❌ 아직 연결되지 않은 패널 임베드입니다.", ephemeral=True)
...
class UnifiedSupportPanelCustomEmbedButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if await reject_if_not_support_template_owner(interaction, "지원서버 임베드 설정"):
            return
        if not self.embed_type:
            return await interaction.response.send_message("❌ 아직 연결되지 않은 지원 서버 임베드입니다.", ephemeral=True)
        await interaction.response.send_modal(
            UnifiedAutoEmbedEditModal(
                interaction.guild.id,
                self.embed_type,
...
async def create_rule_category_channels(guild: discord.Guild):
    """📌 필독 카테고리와 필독/규칙/인사-퇴장 채널을 생성하고 권한을 설정합니다."""
...
@app_commands.choices(생성범위=[
    ...
    app_commands.Choice(name="봇명령어/상점", value="bot_shop"),
    app_commands.Choice(name="뮤직", value="music"),
    app_commands.Choice(name="TTS", value="tts"),
    app_commands.Choice(name="개발로그", value="devlog"),
])
async def create_friend_server(interaction: discord.Interaction, 생성범위: app_commands.Choice[str] = None):
    if await reject_if_not_setup_manager(interaction):
        return
    if await reject_if_not_bot_manager(interaction):
        return
...
async def create_default_server_categories(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        ...
        f"📌 필독 채널: {rule_channel.mention} / 규칙 채널: {rules_channel.mention} / 부스트알림: {boost_channel.mention if boost_channel else '미설정'} / 규칙 안내: {'새로 전송됨' if rules_message_sent else '이미 있음'}\n"
...
async def on_member_join(member: discord.Member):
    ...
    if channel:
        try:
            await channel.send(content=f"🌸 {member.mention}", embed=build_welcome_embed(member, test=False))
...
@bot.tree.command(name="환영보내기", description="자동이 아니라 원하는 때에만 환영 임베드를 직접 보냅니다.")
...
async def send_welcome_manual(
    ...
    if not isinstance(target_channel, discord.TextChannel):
        return await interaction.response.send_message("❌ 텍스트 채널에서만 보낼 수 있습니다.", ephemeral=True)
...
async def enhance_slash_item(interaction: discord.Interaction, 이름: str):
    ...
    if remaining > 0:
        return await interaction.response.send_message(f"⏳ 강화는 1분에 한 번만 가능합니다. 최대 +150강, 기록은 2주마다 초기화됩니다. **{remaining}초** 뒤에 다시 시도해주세요.", ephemeral=True)
...
async def use_title_ticket(interaction: discord.Interaction, 칭호: str):
    ...
    await interaction.response.send_message(f"✅ 칭호 {role.mention} 를 지급하고 `/칭호` 대표 칭호로 장착했습니다!", ephemeral=True)
...
class BuskingPanelView(discord.ui.View):
    ...
    async def _require_voice_client(self, interaction: discord.Interaction):
        ...
        voice_client = interaction.guild.voice_client
        if voice_client is None:
            await interaction.response.send_message("❌ 아직 봇이 음성 채널에 없어요. 먼저 `MR 바로 틀기`를 눌러주세요.", ephemeral=True)
            return None
        return voice_client
    ...
    async def pause_mr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if vc.is_playing():
            vc.pause()
            return await interaction.response.send_message("⏸️ 버스킹 MR을 일시정지했어요.", ephemeral=True)
        await interaction.response.send_message("❌ 현재 재생 중인 MR이 없어요.", ephemeral=True)
    ...
    async def resume_mr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if vc.is_paused():
            vc.resume()
            return await interaction.response.send_message("▶️ 버스킹 MR을 다시 재생했어요.", ephemeral=True)
        await interaction.response.send_message("❌ 일시정지된 MR이 없어요.", ephemeral=True)
    ...
    async def skip_mr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not (vc.is_playing() or vc.is_paused()):
            return await interaction.response.send_message("❌ 스킵할 MR이 없어요.", ephemeral=True)
        vc.stop()
        await interaction.response.send_message("⏭️ 현재 MR을 스킵했어요.", ephemeral=True)
    ...
    async def queue_mr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if queue:
            ...
            text += "대기 중인 MR이 없어요."
        await interaction.response.send_message(embed=create_embed("📜 버스킹 MR 대기열", text[:4000]), ephemeral=True)
    ...
    async def leave_mr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if vc:
            ...
            skip_vote_users.pop(interaction.guild.id, None)
            await vc.disconnect()
            return await interaction.response.send_message("❌ 버스킹 MR을 종료하고 음성 채널에서 나갔어요.", ephemeral=True)
        await interaction.response.send_message("❌ 봇이 음성 채널에 없어요.", ephemeral=True)
...
async def removed_busking_panel(interaction: discord.Interaction, mr: str = None):
    ...
    if is_busking_related_channel(interaction.channel):
        await interaction.response.send_message(embed=embed, view=BuskingPanelView())
    else:
        await interaction.response.send_message(
...
def build_scrim_embed(config: dict, guild: discord.Guild = None, members=None, preview: bool = False):
    ...
    description = (
        f"{description}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎮 게임: **{config.get('game_name') or '종합게임'}**\n"
        f"📌 상태: **{status}**\n"
        f"👥 현재 인원: **{len(members)} / {max_members}명**\n\n"
        f"**참가자 목록**\n{member_text}"
    )[:4096]
...
class ScrimContentModal(discord.ui.Modal, title="내전 모집 만들기/수정"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=build_scrim_embed(config, interaction.guild, preview=True), view=view, ephemeral=True)
...
class ScrimStyleModal(discord.ui.Modal, title="내전 모집 디자인 수정"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(embed=build_scrim_embed(config, interaction.guild, preview=True), view=view)
...
class ScrimDraftView(discord.ui.View):
    ...
    async def _is_owner(self, interaction: discord.Interaction):
        if interaction.user.id == self.owner_id or await can_manage_bot_in_guild(interaction):
            return True
        await interaction.response.send_message("❌ 이 내전 모집 초안을 만든 사람만 수정/전송할 수 있어요.", ephemeral=True)
        return False
    ...
    async def send_scrim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.send_message("❌ 이 채널에 메시지를 보낼 수 없어요. 봇 권한을 확인해주세요.", ephemeral=True)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(content="🗑️ 내전 모집 만들기를 취소했어요.", embed=None, view=self)


@bot.tree.command(name="내전", description="내전 모집을 박스형으로 직접 꾸민 뒤 전송합니다.")
...
@bot.tree.command(name="지원서버템플릿", description="만능 봇 공식 지원 서버 템플릿을 선택해서 자동 생성합니다.")
...
async def send_unified_panel_open(interaction: discord.Interaction, panel_value: str, *, send_to_channel: bool = False):
    ...
    if panel_value == "economy":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 경제 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "mission":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 미션 패널을 현재 채널에 전송했습니다.", ephemeral=True)
        return await interaction.response.send_message(embed=embed, view=MissionPanelView(), ephemeral=True)

    if panel_value == "coupon":
        embed = build_coupon_panel_embed(guild)
        if send_to_channel:
            await channel.send(embed=embed, view=CouponUsePanelView())
            return await interaction.response.send_message("✅ 쿠폰 패널을 현재 채널에 전송했습니다.", ephemeral=True)
        return await interaction.response.send_message(embed=embed, view=CouponUsePanelView(), ephemeral=True)
    ...
    if panel_value == "warning":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 경고 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "staff_tools":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 관리도구 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "admin":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 관리자 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "event":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 이벤트 생성 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "event_end":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 이벤트 종료 패널을 현재 채널에 전송했습니다.", ephemeral=True)
    ...
    if panel_value == "title":
        ...
        if send_to_channel:
            ...
            return await interaction.response.send_message("✅ 칭호 패널을 현재 채널에 전송했습니다.", ephemeral=True)
...
class UnifiedPanelHomeView(discord.ui.View):
    ...
    async def embed_settings(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(embed=embed, view=UnifiedEmbedSettingView(0))
...
def build_mission_panel_embed(guild: discord.Guild, user: discord.User):
    ...
    embed = discord.Embed(
        title=render_auto_embed_text(config["title"], guild)[:256],
        description=(
            f"{render_auto_embed_text(config['description'], guild)}\n\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"📌 **일일미션**: {daily['title']}\n"
            f"• 채팅 {daily['chat_target']}회"
            f"{' + 출석 1회' if daily['attendance_required'] else ''}\n"
            f"• 보상: {daily['reward_point']:,}P + 랜덤박스 {daily['reward_box']}개\n\n"
            f"🗓️ **주간미션**: {weekly['title']}\n"
...
async def send_daily_mission_status(interaction: discord.Interaction, user: discord.User = None):
    ...
    await interaction.response.send_message(embed=embed, view=MissionPanelView(), ephemeral=True)
...
class MissionSettingsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.mission_kind == "weekly":
            ...
            label = "일일미션"
        conn.commit()
        await interaction.response.send_message(f"✅ {label} 설정을 저장했어요. `/미션`에서 바로 확인할 수 있습니다.", ephemeral=True)
...
async def claim_daily_mission_reward_interaction(interaction: discord.Interaction):
    ...
    if settings["reward_box"] > 0:
        c.execute("INSERT INTO inventory VALUES (?, ?, ?)", (user_id, "랜덤박스", settings["reward_box"]))
    c.execute(
        """
        UPDATE daily_mission_progress
...
async def claim_weekly_mission_reward_interaction(interaction: discord.Interaction):
    ...
    if claimed:
        return await interaction.response.send_message("❌ 이번 주 미션 보상은 이미 받았습니다.", ephemeral=True)
...
def build_coupon_panel_embed(guild: discord.Guild):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            "🧾 **사용 방법**\n"
            "아래 `쿠폰 코드 입력` 버튼을 누르고 박스에 쿠폰코드를 입력하세요.\n\n"
            "🛠️ **관리자 기능**\n"
            "쿠폰 생성, 목록 확인, 비활성화도 버튼으로 관리할 수 있습니다.\n"
            "━━━━━━━━━━━━━━━━━━"
        )[:4096],
        color=normalize_embed_color(config["color"])
    )
...
async def send_coupon_panel(interaction: discord.Interaction):
    if interaction.guild is None:
        return await interaction.response.send_message("❌ 서버에서만 사용할 수 있습니다.", ephemeral=True)
    await interaction.response.send_message(embed=build_coupon_panel_embed(interaction.guild), view=CouponUsePanelView(), ephemeral=True)
...
async def process_coupon_use(interaction: discord.Interaction, raw_code: str):
    ...
    if not code:
        return await interaction.response.send_message("❌ 쿠폰 코드를 입력해주세요.", ephemeral=True)
...
@bot.tree.command(name="쿠폰사용", description="박스형 입력창으로 쿠폰 코드를 사용해서 보상을 받습니다.")
...
class FriendEmbedItemButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if kind == "auto":
            ...
            if not embed_type:
                return await interaction.response.send_message("❌ 연결되지 않은 자동 임베드입니다.", ephemeral=True)
            return await interaction.response.send_modal(
        ...
        await interaction.response.send_message("❌ 알 수 없는 임베드 항목입니다.", ephemeral=True)
...
class SupportEmbedItemButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if await reject_if_not_support_embed_owner(interaction):
            return
        if not self.embed_type:
            return await interaction.response.send_message("❌ 아직 연결되지 않은 지원서버 임베드입니다.", ephemeral=True)
        await interaction.response.send_modal(
            UnifiedAutoEmbedEditModal(
                interaction.guild.id,
                self.embed_type,
...
class BackToRootButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="⭐ 만능 봇 | 카테고리형 임베드 설정")
        await interaction.response.edit_message(embed=embed, view=UnifiedEmbedSettingView(self.target_channel_id))
...
class BackToFriendCategoriesButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(embed=embed, view=FriendCategoryView(self.target_channel_id))
...
class BackToSupportCategoriesButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(embed=embed, view=SupportCategoryView(self.target_channel_id))
    ...
    try:
        ...
        try:
            if interaction.response.is_done():
                ...
                await interaction.response.send_message(f"❌ /커스텀 패널 전송 실패: `{e}`", ephemeral=True)
        except Exception:
            pass
